...
MLToolkit - набор инструментов, целью которого является объединение моделей и транзакционной среды, чтобы построенные модели можно было легко использовать прямо в ваших бизнес-процессах. Python Gateway является частью MLToolkit и обеспечивает интеграцию с языком Python предоставляя возможность использовать Python прямо в InterSystems IRIS, выполняя произвольный Python код, осуществляя двунаправленную передачу ранных и обогащая ваши интеллектуальные бизнес-процессы моделями машинного обучения. 
